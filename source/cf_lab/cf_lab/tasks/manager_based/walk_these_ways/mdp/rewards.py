...
# ---------------------------------------------------------------------------
# WTW command index constants (8D behavior vector)
# [0]=theta1, [1]=theta2, [2]=theta3, [3]=frequency,
# [4]=base_height, [5]=body_pitch, [6]=stance_width, [7]=footswing_height
# ---------------------------------------------------------------------------
IDX_THETA1 = 0
IDX_THETA2 = 1
IDX_THETA3 = 2
...
IDX_BODY_PITCH = 5
IDX_STANCE_WIDTH = 6
IDX_FOOTSWING_HEIGHT = 7


def compute_per_foot_timings(
    theta1: torch.Tensor, theta2: torch.Tensor, theta3: torch.Tensor, t: torch.Tensor
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """Compute per-foot timing from the offset parameterization.

    Mapping (Go1 naming -> AYG URDF order):
        FR = t + theta2 + theta3  ->  RF (index 1)
        FL = t + theta1 + theta3  ->  LF (index 0)
        RR = t + theta1           ->  RH (index 3)
        RL = t + theta2           ->  LH (index 2)

    Returns:
        (t_LF, t_RF, t_LH, t_RH) each shape (num_envs,), wrapped to [0, 1).
    """
    t_LF = torch.remainder(t + theta1 + theta3, 1.0)
    t_RF = torch.remainder(t + theta2 + theta3, 1.0)
    t_LH = torch.remainder(t + theta2, 1.0)
    t_RH = torch.remainder(t + theta1, 1.0)
...
class GaitRewardQuad(ManagerTermBase):
    ...
    def compute_contact_targets(self, gait_params):
        ...
        Per-foot timing is computed from theta1/theta2/theta3 offsets, then the contact
...
class FootSwingHeightQuad(GaitRewardQuad):
    ...
    def compute_footswing_height(self, desired_contacts):
        ...
        cmd_height = commands[:, IDX_FOOTSWING_HEIGHT].unsqueeze(1)  # (N, 1)
...
def foot_clearance(
    ...
    commands = env.command_manager.get_command("gait_command")
    cmd_height = commands[:, IDX_FOOTSWING_HEIGHT]
...
def foot_clearance_reward(
    ...
    commands = env.command_manager.get_command("gait_command")
    cmd_height = commands[:, IDX_FOOTSWING_HEIGHT].unsqueeze(1)
...
class RaibertHeuristicFootswing(ManagerTermBase):
    ...
    to the nominal stance based on velocity command and stance width command.
    ...
    def __call__(self, env: ManagerBasedRLEnv, asset_cfg) -> torch.Tensor:
        ...
        # Desired foot y position: adjust default y by stance_width command
        # Compute the stance width adjustment relative to default stance width
        # LF(0), LH(2) are left (+y), RF(1), RH(3) are right (-y)
        default_y = default_pos[:, :, 1]  # (N, 4)
        # Compute current default stance half-width (average absolute y of left/right feet)
        default_half_width = (torch.abs(default_y[:, 0]) + torch.abs(default_y[:, 1])) * 0.5  # (N,)
        # Scale factor: desired half-width / default half-width
        desired_half_width = stance_width_cmd * 0.5  # (N,)
        scale = desired_half_width / torch.clamp(default_half_width, min=0.01)  # (N,)
        p_des_y = default_y * scale.unsqueeze(1)  # Scale all feet proportionally
