...
class AygCommandsCfg:
    ...
    base_velocity = mdp.UniformVelocityCommandCfg(
        ...
        ranges=mdp.UniformVelocityCommandCfg.Ranges(
            lin_vel_x=(-2.0, 3.0), lin_vel_y=(-1.5, 1.5), ang_vel_z=(-2.0, 2.0), heading=(-math.pi, math.pi)
        ),
    )


@configclass
class AygObservationsCfg:
...
class AygRewardsCfg:
    ...
    # -- penalties
    action_smoothness = RewardTermCfg(func=ayg_mdp.action_smoothness_penalty, weight=-1.0)
    air_time_variance = RewardTermCfg(
        func=ayg_mdp.air_time_variance_penalty,
        weight=-1.0,
        params={"sensor_cfg": SceneEntityCfg("contact_forces", body_names=Params.feet_names)},
    )
    base_motion = RewardTermCfg(
        func=ayg_mdp.base_motion_penalty, weight=-2.0, params={"asset_cfg": SceneEntityCfg("robot")}
    )
    base_orientation = RewardTermCfg(
        func=ayg_mdp.base_orientation_penalty, weight=-3.0, params={"asset_cfg": SceneEntityCfg("robot")}
    )
    foot_slip = RewardTermCfg(
        func=ayg_mdp.foot_slip_penalty,
        weight=-0.5,
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names=Params.feet_names),
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=Params.feet_names),
    ...
    joint_torques = RewardTermCfg(
        func=ayg_mdp.joint_torques_penalty,
        weight=-5.0e-4,
        params={"asset_cfg": SceneEntityCfg("robot", joint_names=".*")},
    )
    joint_vel = RewardTermCfg(
        func=ayg_mdp.joint_velocity_penalty,
        weight=-1.0e-2,
        params={"asset_cfg": SceneEntityCfg("robot", joint_names=[".*_HAA", ".*_HFE"])},
    )
...
class AygFlatEnvCfg(LocomotionVelocityRoughEnvCfg):
    ...
    commands: AygCommandsCfg = AygCommandsCfg()

    # MDP setting
    rewards: AygRewardsCfg = AygRewardsCfg()
    terminations: AygTerminationsCfg = AygTerminationsCfg()
    events: AygEventCfg = AygEventCfg()
...
class AygFlatEnvCfg_PLAY(AygFlatEnvCfg):
